 ...
 * (spec §9.2), as read by the poller (the tier-2 `prPolling` gate) and the
 * launcher (fork-PR enforcement, spec §13.1a). Parsing is deliberately
 * tolerant — a partial or unreadable document degrades to the defaults
 * instead of wedging polling on a bad write, and unrelated fields
 * (`secretsAllowedRefs`, `ecrPullRepos`) pass through untouched.
 */
