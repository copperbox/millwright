...
export interface LsRefsOptions {
  ...
  /** agent capability sent with the request. @default millwright-cli */
...
export function lsRefs(stream: UploadPackStream, options: LsRefsOptions): Promise<LsRefsResult> {
  const agent = options.agent ?? 'millwright-cli';
...
export interface DefaultBranchHead {
  /** Short default-branch name, when the server reported HEAD's symref. */
  readonly branch?: string;
  /** Full ref name of the default branch. */
  readonly ref?: string;
  /** Head commit sha; absent when the repository is empty. */
  readonly sha?: string;
  /** True when the repository has no commits on its default branch. */
  readonly empty: boolean;
}

/** Interpret an ls-refs result queried with the `HEAD` prefix. */
export function resolveDefaultBranchHead(result: LsRefsResult): DefaultBranchHead {
  const head = result.refs.find((ref) => ref.name === 'HEAD');
  if (!head) {
    return { empty: true };
  }
  const ref = head.symrefTarget;
  const branch = ref?.startsWith('refs/heads/') ? ref.slice('refs/heads/'.length) : undefined;
  if (head.unborn || !head.sha) {
    return { ...(branch ? { branch } : {}), ...(ref ? { ref } : {}), empty: true };
  }
  return { ...(branch ? { branch } : {}), ...(ref ? { ref } : {}), sha: head.sha, empty: false };
}
